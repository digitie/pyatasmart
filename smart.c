...
static void _disk_dump_attributes(SkDisk *d, const SkSmartAttributeParsedData *a, void* userdata) {
    PyObject* attr_list = userdata;
    PyObject* dict = NULL;

    if (!PyList_CheckExact(attr_list))
    {
            PyErr_SetString(Smart_error, "List Expected");
            return;
    }
...
static PyObject* Smart_get_attributes(Smart* self)
{
    ...
    attr_list = PyList_New(0);

    if ((ret = sk_disk_smart_parse_attributes(self->d, _disk_dump_attributes, attr_list)) < 0)
    {
...
static PyMethodDef Smart_methods[] = {
    ...
    { "close", (PyCFunction)Smart_dealloc, METH_NOARGS, "Close device" },
    { NULL, NULL, 0, NULL }
};
...
PyMODINIT_FUNC initsmart(void)
{
    PyObject* module;

    PyType_Ready(&PyType_Smart);

    module = Py_InitModule3("smart", NULL, SMART_DOC_STRING);
    Smart_error = PyErr_NewException("smart.error", NULL, NULL);
